defmodule Monitrage do
  ...
  @supported_exchange [
    ...
    okex: Monitrage.Okex,
    hitbtc: Monitrage.Hitbtc,
    bitz: Monitrage.Bitz,  
    rightbtc: Monitrage.Rightbtc,
    bitmart: Monitrage.Bitmart,
    coinegg: Monitrage.Coinegg,
    kraken: Monitrage.Kraken     
  ]
  ...
  @supported_exchange
    ...
    |> Enum.map(fn {k, v} -> {k, Task.await(v)} end)
    |> Enum.filter(fn {k,v} -> not match?(%{highest_bid: 0, lowest_ask: nil}, v) end)  
    |> Enum.into(%{})
  end  
